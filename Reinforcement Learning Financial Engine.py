...
directory_path = "C:\\Users\\wadol\\Downloads\\QUANTENGINE"
...
data = pd.read_csv('C:/Users/wadol/Downloads/QUANTENGINE/archive/Stocks/goog.us.txt')
...
def train_dqn(env):
    ...
    for epoch in range(epoch_num):
        ...
        if np.random.rand() > epsilon:
            ...
            pact = np.argmax(q_values.data)
            # Take action
            obs, reward, done = env.step(pact)

            # Add experience to memory
            memory.append((pobs.astype(np.float32), int(pact), float(reward), obs.astype(np.float32), bool(done)))
        if len(memory) > memory_size:
            ...
            if len(memory) == memory_size:
                ...
                for i in memory_idx[::batch_size]:
                    ...
                    b_pobs = np.vstack([item[0] for item in batch])
                    b_pact = np.array([item[1] for item in batch], dtype=np.int32)
                    b_reward = np.array([item[2] for item in batch], dtype=np.float32)
                    b_obs = np.vstack([item[3] for item in batch])
                    b_done = np.array([item[4] for item in batch], dtype=np.bool)
                    q = Q(b_pobs)
                    maxq = np.max(Q_ast(b_obs).data, axis=1)
                    target = np.copy(q.data)
                    for j in range(batch_size):
                            target[j, b_pact[j]] = b_reward[j] + gamma * maxq[j] * (~b_done[j])
            ...
            # Update step and total reward
            total_reward += reward
            pobs = obs
            step += 1
            total_step += 1

        # Store total rewards and losses
    ...
    # Return the final Q-Network and training statistics
    return Q, total_losses, total_rewards
cProfile.run('train_dqn(Environment1(train))')
